...
def create_agents_for_municipality(municipality_class, num_agents):
    agents = []
    for agent_data in municipality_class.agents_data[:num_agents]:
    ...
    return agents  # Adjust to create agents based on percentage
...
for index, row in data.iterrows():
    ...
    elif socio_economic_class == '3':
        socio_economic_weights = [0.2, 0.2, 0.6]  # Middle socio-economic class has a weight of 0.6
    else:
        socio_economic_weights = [0.2, 0.6, 0.2]  # High socio-economic class has a weight of 0.6
...
for index, row in data.iterrows():
    ...
    fertility_rate = row['fertilitetsrate']
    initial_population_percentage = row['Percent'] 
    socio_economic_class = str(row['econ_score'])  # Assuming 'Socio-economic Class' is the column name
...
print(f' The amount of agents generated is : {len(agents_df)}')
#print(agents_df['socio_economic_class'].value_counts(normalize=True))
...
for year in range(0, 34):
    ...
    agents_df_10k.to_csv(f'data_wo_bonus_2205.csv', index=False)

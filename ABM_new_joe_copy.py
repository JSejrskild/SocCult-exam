...
def simulate_child_birth(agents_df, year):
    ...
    for index, agent in agents_df.iterrows():
        ...
        socio_economic_class = str(agent['socio_economic_class'])  # Ensure class is treated as a string
        age = agent['Age']
        children = agent['Children']
        ...
        base_probability = probability_weights.get(socio_economic_class, {}).get(age_bin, 0)
        probability = base_probability + broody
        
        # Debugging statements to check values
        if age_bin == '45-49' and probability > 0:
            print(f"Debug: Age {age}, Age Bin {age_bin}, Socio-economic Class {socio_economic_class}, Children {children}, Base Probability {base_probability}, Broody {broody}, Probability {probability}")
        ...
        agents_df.at[index, f'Year_{year}'] = 1 if has_child else 0
...
agents_df_100.to_csv(f'agents_data_40y2.csv', index=False)
